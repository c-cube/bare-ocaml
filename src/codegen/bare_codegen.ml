...
module CG : sig
  ...
  let add_prelude ~filenames ~standalone self =
    fpf self.out
      "(* generated from %s using bare-codegen *)@.[@@@@@@ocaml.warning \
       \"-26-27\"]@."
      (String.concat "," @@ List.map (Printf.sprintf "%S") filenames);
    if standalone then
      fpf self.out
        "@.(* embedded runtime library *)@.@[<v>@[<2>module Bare = struct@ \
         %s@]@ end@]@.@."
        Embeded_lib.code
    else
...
let () =
  ...
  Arg.parse opts
    (fun f -> files := f :: !files)
    "usage: bare-codegen [opt]* file+";
  let files = List.rev !files in
  let tys = List.map parse_file files |> List.flatten in
  if !cat then List.iter (fun td -> Format.printf "%a@.@." A.pp_ty_def td) tys;
  if !stdout || !out <> "" then
    codegen ~standalone:!standalone ~to_stdout:!stdout ~pp:!pp ~out:!out ~files
      tys;
  ()
