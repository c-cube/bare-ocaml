...
module Find_scc : sig
  ...
  let top (l : ty_def list) : _ list list =
    ...
    let graph =
      List.map (fun d -> d.name, d) l |> List.to_seq |> Hashtbl.of_seq
    in
...
let pplist sep ppx out l =
  Format.pp_print_list
    ~pp_sep:(fun out () -> Format.fprintf out " %s@ " sep)
    ppx out l
...
module P = struct
  ...
  let pp_loc out loc =
    ...
    Format.fprintf out "%d:%d to %d:%d" (line loc.start) (col loc.start)
