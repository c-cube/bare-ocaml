...
let coeff = (try Sys.getenv "COEFF" |> int_of_string with _ -> 10) (* objects per string *)
...
let () =
  ...
  Printf.printf "do %d iterations, %d objects each (each object is %d bytes)\n%!" n coeff len;
  ...
  begin
    ...
    Printf.printf "written %.3f GB in %.2fs (%.3fGB/s)\n%!"
      (written *. 1e-9) dur (written *. 1e-9 /. dur);
  end;
